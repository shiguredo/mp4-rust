...
- [FIX] 破損した MP4 データをデマルチプレックスする際に無限ループが発生する問題を修正する
  - `handle_input()` で同じ `InputRequired` エラーが返り、かつ前回提供されたデータが要求を満たしていた場合、破損データによる進行不能状態として検出しエラーを返す
  - @voluntas
- [FIX] HvccBox と VpccBox のデコード時にスライス境界チェックを追加する
