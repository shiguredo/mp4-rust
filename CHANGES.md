...
- [FIX] 入力データが破損してる場合に `DecoderSpecificInfo::decode()` で算術オーバーフローが発生する可能性がある問題を修正する
  - 入力データからデコードしたサイズ値が極端に大きい場合に加算がオーバーフローする可能性があった
  - `saturating_add()` を使うようにする
    - 飽和した場合は、その後のバッファとのサイズチェック部分でエラーとなる（これは期待通りの挙動）
  - @sile
